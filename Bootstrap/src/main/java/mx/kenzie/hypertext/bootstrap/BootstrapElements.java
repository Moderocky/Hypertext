...
public interface BootstrapElements extends StandardElements {
    ...
    HTMElement
        ...
        META_VIEWPORT = new HTMElement("meta").single()
                                              ...
                                              .finalise(),
        ALERT = StandardElements.DIV.set("role", "alert").classes("alert").finalise(),
        BADGE = StandardElements.SPAN.classes("badge").finalise(),
        BREADCRUMBS = new MultiElement(
            StandardElements.NAV.set("aria-label", "breadcrumb"),
            StandardElements.OL.classes("breadcrumb")
        ).finalise(),
        BREADCRUMB = StandardElements.LI.classes("breadcrumb-item").finalise(),
        BUTTON = StandardElements.BUTTON.set("type", "button").classes("btn").finalise(),
        BUTTON_GROUP = StandardElements.DIV.set("role", "group").classes("btn-group").finalise();
